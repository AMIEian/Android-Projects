...
ini_set('memory_limit', '1024M');
	...
	$customer_product_data_name = 'dadar_product_data';
	$customer_stock_data_name = 'dadar_stock_data';
...
//echo "SELECT  $customer_product_data_name.*, $customer_stock_data_name.*  FROM $customer_stock_data_name  INNER JOIN $customer_product_data_name ON $customer_product_data_name.barcode = $customer_stock_data_name.barcode    WHERE $customer_stock_data_name.store_id  ='$store_id' AND $customer_stock_data_name.customer_id = $customer_id ORDER BY $customer_stock_data_name.stock_id ASC";
	...
	//echo "SELECT  $customer_stock_data_name.barcode, tagsmart_stock_details.*, epc_location_date_time.* , tagsmart_stock_details.tagsmart_id FROM $customer_stock_data_name  INNER JOIN tagsmart_stock_details ON tagsmart_stock_details.stock_id = $customer_stock_data_name.stock_id LEFT JOIN epc_location_date_time ON tagsmart_stock_details.tagsmart_id = epc_location_date_time.tagsmart_id    WHERE $customer_stock_data_name.store_id  ='$store_id' AND $customer_stock_data_name.customer_id = $customer_id";
