<?php
	...
	/*$testStr ='["8905014479823","8905014479830","8905014259616"]';
    ...
    $selectedIDs = rtrim($temp, ",");
   	...
   	{
   		$query_str="select Distinct  ".$str.", ".$customer_stock_data_table_name.".barcode, ".$customer_stock_data_table_name.".quantity, CASE WHEN replenishment_data.manual_replenishment_quantity ='0' THEN replenishment_quantity::int WHEN replenishment_quantity='0' THEN manual_replenishment_quantity::int ELSE least(manual_replenishment_quantity::int,replenishment_quantity::int) END AS replenishment_quantity,sub_location_master.sublocation, epc_location_date_time.sub_location_id, epc FROM ". $customer_product_data_table_name." INNER JOIN ".$customer_stock_data_table_name." ON ".$customer_stock_data_table_name.".barcode = ".$customer_product_data_table_name.".barcode Inner JOIN tagsmart_stock_details ON ".$customer_stock_data_table_name.".stock_id = tagsmart_stock_details. stock_id Inner JOIN epc_location_date_time ON tagsmart_stock_details.tagsmart_id = epc_location_date_time. tagsmart_id INNER JOIN replenishment_data ON replenishment_data.barcode = ".$customer_product_data_table_name.".barcode INNER JOIN sub_location_master ON sub_location_master.sub_location_id = epc_location_date_time.sub_location_id WHERE sldt_id IN (".$selectedIDs.") ";
	        ...
	        {       
	           // print_r( $row6 );
				...
				else //if barcode is unique in the barcode array
				{
					...
					$mainResponse[$counter]['replenishment_quantity'] = $row6['replenishment_quantity'];
