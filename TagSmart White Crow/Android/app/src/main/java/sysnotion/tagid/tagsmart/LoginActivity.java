...
public class LoginActivity extends AppCompatActivity {
            ...
            public void onClick(View view) {
                if(editTextTextEmailAddress.getText().length() >0 && editTextTextPassword.getText().length()>0) {
                    ...
                    if(Constants.isNetworkAvailable(LoginActivity.this)) {
                        ...
                        new UserValidationAsyncTask_2(LoginActivity.this).execute(Constants.LOGIN_URL_JSON);
                    }
                    else
                    {
    ...
    class UserValidationAsyncTask_2 extends AsyncTask<String, String, String> {

        OkHttpClient client;
        private ProgressDialog dialog;

        public UserValidationAsyncTask_2(Activity activity) {
            dialog = new ProgressDialog(activity);
        }
        ...
        protected void onPostExecute(String result) {
            ...
            super.onPostExecute(result);
            dialog.dismiss();
            System.out.println("Response UserValidationAsyncTask_2 "+result);
            ...
            if (result != null) {
                try
                {
                    ...
                    if(jObj.getString("details") == "success") {
                        ...
                        SharedPreferences.Editor edit = pref.edit();
                        edit.clear();
                        edit.putInt(Constants.USER_ID_NO, data.getInt("user_id"));
                        edit.putString(Constants.EMAIL, data.getString("email"));
                        edit.putString(Constants.LAST_LOGIN, data.getString("last_login"));
                        edit.putInt(Constants.STORE_ID_NO, data.getInt("store_id"));
                        edit.putString(Constants.UPDATED_TIME, data.getString("updated_time"));
                        edit.putString(Constants.PASSWORD, data.getString("password"));
                        edit.putString(Constants.USERNAME, data.getString("username"));
                        edit.putString(Constants.USER_ROLE, data.getString("user_role"));
