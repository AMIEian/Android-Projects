...
public class SearchDataImporterActivity extends AppCompatActivity {
    private DBManager dbManager;
    SharedPreferences pref;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_search_data_importer);
        pref = getApplicationContext().getSharedPreferences(Constants.PREFS_NAME, MODE_PRIVATE);
        dbManager = new DBManager(SearchDataImporterActivity.this);
        dbManager.open();
        //important to set category level to 1
        Constants.CATEGORY_LEVEL=0;

        if(Constants.isNetworkAvailable(SearchDataImporterActivity.this)) {
            new  SQLiteTableDataAsyncTask(SearchDataImporterActivity.this).execute(Constants.BASE_URL + "GetProductStockTagsmartEPCTableData.php");
        }
        else
        {
            Constants.internetAlert(SearchDataImporterActivity.this);
        }
    }
        ...
        protected void onPostExecute(String result) {
            ...
            if (result != null) {
                try
                {
                    ...
                    //Stock Take A option
                  /*  Intent intent = new Intent(DataImporterActivity.this , StockTakeOptionACategoryActivity.class);

                    startActivity(intent);
                    finish();*/

                    //Stock Take B option
                    Intent intent = new Intent(SearchDataImporterActivity.this , SearchDashboardActivity.class);
                    ...
                    Toast.makeText(SearchDataImporterActivity.this, "Something went wrong. Please try again", Toast.LENGTH_LONG).show();
                    finish();
                }
